...
if __name__ == '__main__':
    ...
    model, end_points = mobilenet_v2(input, num_classes=1001, is_training=True)
    print(model, end_points)

    fake_data = np.ones(shape=(1, 224, 224, 3))

    sess_config = tf.ConfigProto()
    init_fn = slim.assign_from_checkpoint_fn(
        'mobilenetv2_sgd/model.ckpt',
        slim.get_model_variables('MobileNetV2'))
    with tf.Session(config=sess_config) as sess:
        ...
        #saver.restore(sess, "mobilenetv2_sgd/model.ckpt-1450000")
        
        output = sess.run(model, feed_dict={input: fake_data})
        print(output, output[0][885], output[0][886], output[0][887])
        ...
        for i in range(101):
            ...
            if i != 0:
                cnt += time.time() - t1
        print("MobileNetV2 Forward time: %s" % (cnt / 100))'''
        
        output_graph_def = graph_util.convert_variables_to_constants(sess, sess.graph_def, output_node_names=["MobilenetV2/Logits/SpatialSqueeze"])
        with tf.gfile.FastGFile("tensorflow_mobilenetv2_graph.pb", mode='wb') as f:
