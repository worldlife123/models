...
def _get_filenames(dataset_dir, split_name):
  ...
  """
  assert split_name in ['train', 'validation_1', 'validation_2', 'validation_3']
  filelist_dict = {
    'train' : '300VW_train_filelist.txt',
    'validation_1' : '300VW_test_category_1_filelist.txt',
    'validation_2' : '300VW_test_category_2_filelist.txt',
    'validation_3' : '300VW_test_category_3_filelist.txt',
  }
...
def _get_dataset_filename(dataset_dir, split_name, shard_id):
  output_filename = 'landmark_300VW_%s_%05d-of-%05d.tfrecord' % (
...
def _dataset_exists(dataset_dir):
  for split_name in ['train', 'validation_1', 'validation_2', 'validation_3']:
    for shard_id in range(_NUM_SHARDS):
      output_filename = _get_dataset_filename(
          dataset_dir, split_name, shard_id)
...
#    return
  ...
  validation_1_filenames, validation_1_label_filenames = _get_filenames(dataset_dir, 'validation_1')
  print("%d images for validation category 1" % len(validation_1_filenames))
  validation_2_filenames, validation_2_label_filenames = _get_filenames(dataset_dir, 'validation_2')
  print("%d images for validation category 2" % len(validation_2_filenames))
  validation_3_filenames, validation_3_label_filenames = _get_filenames(dataset_dir, 'validation_3')
  print("%d images for validation category 3" % len(validation_3_filenames))

  # First, convert the training and validation sets.
  _convert_dataset('train', training_filenames, training_label_filenames,
                   dataset_dir, writer)
  _convert_dataset('validation_1', validation_1_filenames, validation_1_label_filenames,
                   dataset_dir, writer)
  _convert_dataset('validation_2', validation_2_filenames, validation_2_label_filenames,
                   dataset_dir, writer)
  _convert_dataset('validation_3', validation_3_filenames, validation_3_label_filenames,
                   dataset_dir, writer)
