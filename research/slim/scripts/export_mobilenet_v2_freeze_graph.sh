...
echo "*******"
echo "Exporting graph architecture to ${MODEL_FOLDER}/frozen_graph.pb"
echo "*******"
python export_inference_graph_with_checkpoint.py \
  --model_name=${SLIM_NAME} --dataset_name=landmark_300W_LP --image_size=${IMAGE_SIZE} --logtostderr \
  --output_file=${MODEL_FOLDER}/frozen_graph.pb --dataset_dir=${MODEL_FOLDER} --checkpoint_dir=${CHECKPOINT} \
  --model_name_scope=${MODEL_SCOPE} --output_node=${OUTPUT_NODE_NAMES}
