...
PRETRAINED_CHECKPOINT_DIR=/tmp/mobilenet_v2_training_quantize/model.ckpt-200000

# Where the training (fine-tuned) checkpoint and logs will be saved to.
TRAIN_DIR=/tmp/mobilenet_v2_training_quantize/ft-custom-fd
...
python train_image_regressor.py \
  --train_dir=${TRAIN_DIR} \
  --dataset_name=landmark_300VW_custom_fd \
  --dataset_split_name=train \
  --dataset_dir=${DATASET_DIR} \
  --model_name=mobilenet_v2 \
  --preprocessing_name face_landmark_custom_fd \
